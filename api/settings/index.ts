  ...
  return withCors(req, res, async () => {
    try {
      ...
      if (req.method === 'PATCH') {
        ...
        const allowed = ['title', 'subtitle', 'results_bg', 'preview_bg', 'victory_bg', 'main_color', 'visual_type', 'category', 'match_date', 'location'] as const;
        const updates = allowed.filter((k) => body[k] !== undefined);
        if (updates.length === 0) {
          const rows = (await sql`SELECT id, title, subtitle, results_bg, preview_bg, victory_bg, main_color, visual_type, category, match_date, location FROM settings WHERE id = 1`) as Record<string, unknown>[];
          const row = rows[0];
        ...
        if (!rows[0]) {
          ...
          if (body.results_bg !== undefined) await sql`UPDATE settings SET results_bg = ${body.results_bg as string}, updated_at = now() WHERE id = 1`;
          if (body.preview_bg !== undefined) await sql`UPDATE settings SET preview_bg = ${body.preview_bg as string}, updated_at = now() WHERE id = 1`;
          if (body.victory_bg !== undefined) await sql`UPDATE settings SET victory_bg = ${body.victory_bg as string}, updated_at = now() WHERE id = 1`;
